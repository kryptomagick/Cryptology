...
void qapla_F(struct qapla_state *state) {
    ...
    for (i = 0; i < state->rounds; i++) {
        state->r[0] += state->r[7];
        state->r[1] = rotl64((state->r[1] ^ state->r[0]), 9);
        state->r[2] += state->r[5];
        state->r[3] = rotl64((state->r[3] ^ state->r[2]), 21);
        state->r[4] += state->r[3];
        state->r[5] = rotl64((state->r[5] ^ state->r[4]), 12);
        state->r[6] += state->r[1];
        state->r[7] = rotl64((state->r[7] ^ state->r[6]), 18);
        state->r[1] += state->r[0];
        state->r[2] = rotl64((state->r[2] ^ state->r[7]), 9);
        state->r[3] += state->r[2];
        state->r[4] = rotl64((state->r[4] ^ state->r[5]), 21);
        state->r[5] += state->r[4];
        state->r[6] = rotl64((state->r[6] ^ state->r[3]), 12);
        state->r[7] += state->r[6];
        state->r[0] = rotl64((state->r[0] ^ state->r[1]), 18);
    }
    for (i = 0; i < 8; i++) {
...
void * qapla_encrypt(char *keyfile1, char *keyfile2, char * inputfile, char *outputfile, int key_length, int nonce_length, int mac_length, int kdf_iterations, unsigned char * kdf_salt, int salt_len, int password_len,  int keywrap_ivlen, int mask_bytes, int bufsize, unsigned char * passphrase) {
    struct qloq_ctx ctx;
...
void * qapla_decrypt(char *keyfile1, char *keyfile2, char * inputfile, char *outputfile, int key_length, int nonce_length, int mac_length, int kdf_iterations, unsigned char * kdf_salt, int salt_len,  int password_len,  int keywrap_ivlen, int mask_bytes, int bufsize, unsigned char * passphrase) {
    struct qloq_ctx ctx;
    BIGNUM *tmp;
    BIGNUM *tmpS;
    BIGNUM *BNctxt;
